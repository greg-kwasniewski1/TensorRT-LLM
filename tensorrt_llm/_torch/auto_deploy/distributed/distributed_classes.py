...
class DistributedTensor:
    ...
    def init_flattening(self):
        ...
        if self.tensor_B is not None:
            self.non_contracted_modes_B = [
                mode
                for mode in self.tensor_B
                if mode in self.tensor_C and mode not in self.tensor_A
            ]
        else:
            self.non_contracted_modes_B = []
        ...
        self.M_global = int(
            np.prod([self.modes_extents[mode] for mode in self.non_contracted_modes_A])
        )
        self.N_global = int(
            np.prod([self.modes_extents[mode] for mode in self.non_contracted_modes_B])
        )
    ...
    def matches(self, other, modes: str):
        ...
        return all(
            self.modes_extents_local[mode] == other.modes_extents_local[mode] for mode in modes
    ...
    def coords_to_global_slice(self, p_coords: dict[str, int] = None) -> dict[str, slice]:
        ...
        for mode in self.extent_order_sorted:
            slices[mode] = (
                p_coords[mode] * self.modes_extents_local[mode],
...
class PGrid:
    ...
    def __init__(
        ...
        nonparallelizable_dim: int = None,
    ):
        ...
        if grid is None:
            self.grid = self.find_optimal_grid(
                M_global, K_global, N_global, self.P, nonparallelizable_dim=nonparallelizable_dim
            )
        else:
            self.grid = grid
    ...
    def matches(self, other):
        ...
        grid_match = self.P_k == other.P_n and self.P_m == other.P_m and self.P_n == other.P_k
    ...
    def find_optimal_grid(self, M: int, K: int, N: int, P: int, nonparallelizable_dim: int = -1):
        V = M * K * N / P
        a = V ** (1 / 3)
        ...
        max_P_m = P + 1 if nonparallelizable_dim != 1 else 1
        max_P_k = P + 1 if nonparallelizable_dim != 2 else 1
    ...
    def coords_to_slice(
        self, grid_coords: tuple[int, int, int] = None, input: bool = False
    ...
    def slice_to_coords(
        self, m_min: int, m_max: int, n_min: int, n_max: int, input: bool = False
    ) -> tuple[int, int, int]:
    ...
    def slice_to_coord(
        self, m_global: int, n_global: int, input: bool = False
    ) -> tuple[int, int, int]:
        """Convert slice to coordinates."""
...
class Distributed3DLinear(nn.Module):
    ...
    def init_local_weight_slice(self, bias: bool = False, dtype: torch.dtype = torch.float16):
        ...
        self.weight = nn.Parameter(
            torch.empty(self.grid.N_global, self.grid.K_global, device=self.device, dtype=dtype)
        )
        if bias:
            self.bias = nn.Parameter(
                torch.empty(self.grid.N_global, device=self.device, dtype=dtype)
            )
        else:
        ...
        # Slice the global weight matrix according to process grid coordinates
        # Reshape weight to [P_k, K//P_k, P_n, N//P_n]
        w_reshaped = self.weight.view(
            self.grid.P_n, self.grid.N_local, self.grid.P_k, self.grid.K_local
        )
        # Get local slice based on process grid coordinates
    ...
    def _init_process_groups(self):
        ...
        for pm in range(self.grid.P_m):
            for pn in range(self.grid.P_n):
                reduction_ranks = [
                    self.grid.coords_to_rank(grid_coords=(pk, pm, pn))
                    for pk in range(self.grid.P_k)
                ]
    ...
    def initialize_weight_matrices(
        self, fixed_init: bool = True, dtype: torch.dtype = torch.float16
    ):
    ...
    def forward(
        self, x_local: torch.Tensor, start_pos=None, mask=None, freqs_cis=None
    ) -> torch.Tensor:
        """
        ...
        if torch.isnan(x_local).any() or torch.isinf(x_local).any():
            log(
                f"Rank {self.grid.rank}, distributed linear layer beginning with nans",
                log_level=CRITICAL,
            )
            exit()
        # if self.grid.rank == 0:
        ...
        log(
            f"Rank {self.grid.rank}: Forward pass after matmul. x_local:\n{x_local.detach().cpu().numpy()}"
        ...
        for group, (pm, pn) in self.reduction_groups:
            if self.grid.pm == pm and self.grid.pn == pn:
                ...
                if dist.get_world_size(group) > 1:
                    ...
                    log(
                        f"Rank {self.grid.rank}: all_reduce comm volume:  {self.comm_vol} += {torch.prod(torch.tensor(y_local.shape))} * 2 * ({dist.get_world_size(group)} - 1)/{dist.get_world_size(group)}",
                        log_level=DEBUG,
                        ranks=[0],
                    )
                    dist.barrier()
                    log(
                        f"Rank {self.grid.rank}, allreduce within group: {group_ranks}, result:\n{y_local.detach().cpu().numpy()}",
                        log_level=VALUES,
                        ranks=[0],
                    )
                    dist.barrier()
                    ...
                    self.comm_vol += (
                        torch.prod(torch.tensor(y_local.shape))
                        * 2
        ...
        if self.bias is not None:
            ...
            bias_local = self.bias[
                self.grid.pn * self.grid.N_local : (self.grid.pn + 1) * self.grid.N_local
            ]
        ...
        if torch.isnan(y_local).any() or torch.isinf(y_local).any():
            log(
                f"Rank {self.grid.rank}, distributed linear layer ending with nans",
                log_level=CRITICAL,
            )
            exit()
        return y_local
    ...
    def reshuffle(self, local_tensor_pre: torch.Tensor) -> torch.Tensor:
        ...
        if local_tensor_pre.device.type == "meta":
            ...
            return torch.empty(
                (self.grid.M_local, self.grid.K_local), device="meta", dtype=local_tensor_pre.dtype
            )
        ...
        if self.grid.matches(self.input_p_grid):
            log("reshuffle grid matches input grid! Nice........\n")
            return local_tensor_pre

        log(
            f"reshuffle grid does not match input grid. current grid: {self.grid}, input grid: {self.input_p_grid}\n"
        )
        assert (
            max(self.grid.M_local, self.input_p_grid.M_local)
            % min(self.grid.M_local, self.input_p_grid.M_local)
            == 0
        ), (
            f"M_local must be divisible by input_p_grid.M_local, M_local: {self.grid.M_local}, input_p_grid.M_local: {self.input_p_grid.M_local}"
        )
        assert (
            max(self.grid.K_local, self.input_p_grid.N_local)
            % min(self.grid.K_local, self.input_p_grid.N_local)
            == 0
        ), (
            f"N_local must be divisible by input_p_grid.N_local, N_local: {self.grid.N_local}, input_p_grid.N_local: {self.input_p_grid.N_local}"
        )
        ...
        local_tensor_rcv = torch.empty(
            m_num_chunks_to_receive,
        ...
        for chunk_m in range(num_chunks_m):
            ...
            for chunk_n in range(num_chunks_n):
                ...
                pm_src, pn_src = self.input_p_grid.slice_to_coord(
                    m_global=m_global, n_global=n_global, input=True
                )
                pm_dst, pk_dst = self.grid.slice_to_coord(
                    m_global=m_global, n_global=n_global, input=False
                )
                ...
                for pn_dst in range(self.grid.P_n):
                    src_rank = self.input_p_grid.coords_to_rank(
                        grid_coords=(
                            (m_chunk_ind + n_chunk_ind + pn_dst) % self.input_p_grid.P_k,
                            pm_src,
                            pn_src,
                        ),
                        input=False,
                    ...
                    if src_rank == self.grid.rank:
                        ...
                        send_op = dist.P2POp(
                            dist.isend,
                            local_tensor_send[
                                chunk_m % m_num_chunks_to_send, chunk_n % n_num_chunks_to_send
                            ],
                            dst_rank,
                        )
                        ops.append(send_op)
                        ...
                        if dst_rank != self.grid.rank:
                            log(
                                f"Rank {self.grid.rank}: Current vol: {self.comm_vol}, reshuffle send volume = {chunk_size_m} * {chunk_size_n}",
                                log_level=DEBUG,
                            )
                    ...
                    if dst_rank == self.grid.rank:
                        ...
                        if not tmp.is_contiguous():
                            print(
                                f"Rank {self.grid.rank}: local_tensor_rcv[m_chunk_ind, n_chunk_ind] is not contiguous! "
                                + f"shape: {tmp.shape}, dtype: {tmp.dtype}, device: {tmp.device}"
                            )
                            exit()
                        recv_op = dist.P2POp(
                            dist.irecv, local_tensor_rcv[m_chunk_ind, n_chunk_ind], src_rank
                        )
                        ops.append(recv_op)
        ...
        local_tensor_post = rearrange(
            local_tensor_rcv,
            "m_b n_b b_m b_n -> (m_b b_m) (n_b b_n)",
    ...
    def output_reshuffle(
        self, local_tensor_pre: torch.Tensor, output_p_grid: PGrid
    ) -> torch.Tensor:
        """
        ...
        if self.grid == output_p_grid:
            log("reshuffle grid matches output grid! Nice........\n")
            return local_tensor_pre

        log(
            f"Output reshuffler. grid does not match output grid. current grid: {self.grid}, output grid: {output_p_grid}\n"
        )
        assert (
            max(self.grid.M_local, output_p_grid.M_local)
            % min(self.grid.M_local, output_p_grid.M_local)
            == 0
        ), (
            f"M_local must be divisible by output_p_grid.M_local, M_local: {self.grid.M_local}, output_p_grid.M_local: {output_p_grid.M_local}"
        )
        assert (
            max(self.grid.N_local, self.input_p_grid.N_local)
            % min(self.grid.N_local, self.input_p_grid.N_local)
            == 0
        ), (
            f"N_local must be divisible by input_p_grid.N_local, N_local: {self.grid.N_local}, input_p_grid.N_local: {self.input_p_grid.N_local}"
        )
        ...
        for chunk_m in range(num_chunks_m):
            ...
            for chunk_n in range(num_chunks_n):
                ...
                pm_src, pn_src = self.grid.slice_to_coord(
                    m_global=m_global, n_global=n_global, input=True
                )
                pm_dst, pn_dst = output_p_grid.slice_to_coord(
                    m_global=m_global, n_global=n_global, input=True
                )
                ...
                for pk_dst in range(output_p_grid.P_k):
                    src_rank = self.grid.coords_to_rank(
                        grid_coords=(
                            (m_chunk_ind + n_chunk_ind + pn_dst) % self.grid.P_k,
                            pm_src,
                            pn_src,
                        ),
                        input=False,
                    ...
                    if src_rank == self.grid.rank:
                        log(
                            f"Output reshuffle: Rank {self.grid.rank}: sending to {dst_rank}: chunk_ind: [{chunk_m}, {chunk_n}], "
                            + f"src_rank: {src_rank}, dst_rank: {dst_rank}, "
                            + f"src_coords: ({m_chunk_ind + n_chunk_ind}, {pm_src}, {pn_src}), "
                            + f"dst_coords: ({pk_dst}, {pm_dst}, {pn_dst}), "
                            + f"grid: [{self.grid.P_k}, {self.grid.P_m}, {self.grid.P_n}], "
                        ...
                        log(
                            f"Output reshuffle SEND. Rank {self.grid.rank}: sending to {dst_rank}: chunk_ind: [{chunk_m}, {chunk_n}], "
                            + f"src_rank: {src_rank}, dst_rank: {dst_rank}, "
                            + f"src_coords: ({m_chunk_ind + n_chunk_ind}, {pm_src}, {pn_src}), "
                            + f"dst_coords: ({pk_dst}, {pm_dst}, {pn_dst}), "
                            + f"grid: [{self.grid.P_k}, {self.grid.P_m}, {self.grid.P_n}], "
                    ...
                    if dst_rank == self.grid.rank:
                        ...
                        log(
                            f"Output reshuffle RECEIVE. Rank {self.grid.rank}: receiving from {src_rank}: chunk_ind: [{chunk_m}, {chunk_n}], "
                            + f"src_rank: {src_rank}, dst_rank: {dst_rank}, "
                            + f"src_coords: ({m_chunk_ind + n_chunk_ind}, {pm_src}, {pn_src}), "
                            + f"dst_coords: ({pk_dst}, {pm_dst}, {pn_dst}), "
                            + f"grid: [{self.grid.P_k}, {self.grid.P_m}, {self.grid.P_n}], "
    ...
    def distribute_input_tensor(
        ...
        log(
            f"Rank {self.grid.rank}, about to reshape X_global from [M, K] = [{M}, {N}] to [P_m, M//P_m, P_k, K//P_k] = [{P_m}, {M // P_m}, {P_k}, {N // P_k}]"
        )
        X_global = rearrange(
            X_global,
        ...
        log(
            f"Rank {self.grid.rank}, distribute input tensor. Shapes: X_global: {X_global.shape}, "
    ...
    def forward(self, x, start_pos, mask, freqs_cis):
        ...
        if isinstance(self.q_proj, Distributed3DLinear):
            ...
            queries = queries.view(
                bsz, seqlen, self.config.n_heads // self.q_p_grid.P_n, self.config.head_dim
            )
            keys = keys.view(
                bsz, seqlen, self.config.n_kv_heads // self.kv_p_grid.P_n, self.config.head_dim
            )
            values = values.view(
                bsz, seqlen, self.config.n_kv_heads // self.kv_p_grid.P_n, self.config.head_dim
            )
        ...
        # # Rotary embeddings
        if not isinstance(x, torch.fx.proxy.Proxy) and not isinstance(
            freqs_cis, torch.fx.proxy.Proxy
        ):
            # try:
        ...
        if not isinstance(out, torch.fx.proxy.Proxy):
            ...
            if torch.isnan(out).any():
                # print datatypes
                log(
                    f"\n\n\n!!!queries.dtype: {queries.dtype}, keys.dtype: {keys.dtype}, values.dtype: {values.dtype}, mask.dtype: {mask.dtype}, out.dtype: {out.dtype}",
                    log_level=DEBUG,
                )
