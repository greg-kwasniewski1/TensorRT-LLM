...
from utils.debug_utils import CRITICAL, DEBUG, INFO, log, visualize_model
...
def distribute_linear_layer(
    ...
    if isinstance(layer, torch.nn.Linear):
        distr_linear = Distributed3DLinear(
            embd_dim=layer.in_features,
...
def parallelize_model(
    ...
    for name, module in linear_layers:
        ...
        log(
            f"Converting layer {name} to Distributed3DLinear (depth {layer_depth}). First, trying to find the predecessors distributed linear layers."
        )
    ...
    log(
        f"Successfully parallelized {len(_distributed_linear_registry)} linear layers",
        log_level=INFO,
    )

    # parallelize th RMSNorm layers
    norm_layers = [
        (name, module) for name, module in model.named_modules() if isinstance(module, RMSNorm)
    ]
    for name, norm_layer in norm_layers:
        ...
        distr_norm_layer = DistributedRMSNorm(
            p_grid=previous_p_grid, weight=norm_layer.weight, norm_eps=norm_layer.norm_eps
        )
    ...
    attn_layers = [
        ...
        and any(hasattr(module, param_name) for param_name in head_param_names)
    ]
        ...
        for head_param_name in head_param_names:
            if hasattr(attn_layer, head_param_name):
                ...
                setattr(
                    attn_layer,
                    head_param_name,
                    getattr(attn_layer, head_param_name) // input_distributer.grid.P,
                )
