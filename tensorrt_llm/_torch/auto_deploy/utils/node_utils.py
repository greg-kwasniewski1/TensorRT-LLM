...
def _extract_aggregation_dimension(node: Node) -> Optional[int]:
    ...
    elif is_op(node, torch.ops.aten.split) or is_op(node, torch.ops.aten.split_with_sizes) or \
        is_op(node, torch.ops.aten.chunk) or is_op(node, torch.ops.aten.slice.Tensor) or \
        is_op(node, torch.ops.aten.cat):
        # split dimension argument is the last one
        return 2 #node.args[-1]
